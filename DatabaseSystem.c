...
void window()
{
	...
	gotoxy(5, 3);
	printf("HAJEE MOHAMMAD DANESH SCIENCE AND TECHNOLOGY UNIVERSITY , DINAJPUR");
	gotoxy(25, 24);
	SetColor(17);
	//printf("jahid");
...
void print_heading(const char st[])
{
    ...
    gotoxy(45,8);printf("SDS : %s",st);
    SetColorAndBackground(17,15);
	
}

int conf_database(char id[])
{
	// it checks whether the enter id for
	// new database is already in the database
}

void add_student(){
	
    clearWindow();
    print_heading("Add Info");
    int print = 37;
    FILE *fp;
    ...
    if(fp == NULL){
        ...
        fflush(stdin);
        gotoxy(print,10); printf("ID: ");
		gets(stu.ID);
        //here you can confirms the ID
        gotoxy(print,12); printf("Name: ");
		gets(stu.name);
        gotoxy(print,14); printf("Department: ");
		gets(stu.Class);
        gotoxy(print,16);printf("Address: ");
		gets(stu.add);
        gotoxy(print,18);printf("Parent's name: ");
		gets(stu.parname);
        gotoxy(print,20);printf("Phone Number: ");
		scanf("%ld",&stu.phone_no);
        fwrite(&stu, sizeof(stu), 1, fp);
        gotoxy(40,22); printf("The Info is sucessfully added");
    }
    SetColor(28);
    fclose(fp);
    return;
...
void search_student()
{
    ...
    clearWindow();
    print_heading("Search Info");
    SetColor(45);
    char s_id[15];
    int isFound = 0;
    gotoxy(37,10);printf("Enter ID to Search: ");
	fflush(stdin);
    gets(s_id);
    FILE *fp;
    ...
    if(isFound == 1){
        gotoxy(37,12);printf("The Info is Found");
        gotoxy(37,14);printf("ID: %s",stu.ID);
        gotoxy(37,15);printf("Name: %s",stu.name);
        gotoxy(37,16);printf("Class: %d",stu.Class);
        gotoxy(37,17);printf("Address: %s",stu.add);
        gotoxy(37,18);printf("Parent's Name: %s",stu.parname);
        gotoxy(37,19);printf("Phone No: %ld",stu.phone_no);
    }else{
        gotoxy(37,12);printf("Sory, No Info found in the Database");
    }
    SetColor(28);
    fclose(fp);
    return;
...
void mod_student()
{
    ...
    while(fread(&stu, sizeof(stu),1,fp) == 1){
        if(strcmp(s_id, stu.ID) == 0){
            ...
            gotoxy(print,14);printf("Class: ");scanf("%d",&stu.Class);
            gotoxy(print,15);printf("Address: ");gets(stu.add);
    ...
    if(!isFound){
        gotoxy(print, 12);printf("No info Found");
    }
    fclose(fp);
    SetColor(28);
    return;
...
void gen_marksheet(){
    ...
    clearWindow();
    print_heading("Mark Sheet");
    gotoxy(37,14);printf("Filed is empty ");
...
/*void delete_student()
{
   ...
   // temp = fopen("temp.txt", "wb");
    ...
    if( fp == NULL)
    {
    	gotoxy(37,12);printf("Info Not Found");
	}
	else{
		temp = fopen("temp.txt", "wb+");
		if( fp == NULL)
   		{
    		gotoxy(37,12);printf("File Opening Error");
		}
		else{
			gotoxy(37,10);printf("Enter ID to Modify: ");
			while(fread(&stu, sizeof(stu),1,fp) == 1)
...
void main_window()
{
    ...
    while(1){
        ...
        gotoxy(x,12);printf("3. Modify Student Info");
        gotoxy(x,14);printf("4. Generate Marksheet");
        gotoxy(x,16);printf("5. Delete Student Info");
...
int main()
{
    ...
    SetConsoleTitle("HLF- Student Record System");
    window();
    //use_pass_field();
    main_window();
